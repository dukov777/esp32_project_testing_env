...
@pytest.mark.parametrize('embedded_services,target,app_path,build_dir', [
    ('idf', 'linux', os.path.join(HERE, 'tests', 'apps', 'test_component_A'), BUILD_LINUX),
...
@pytest.mark.host
@pytest.mark.parametrize('embedded_services,target,app_path,build_dir', [
    ('idf', 'linux', os.path.join(HERE, 'tests', 'apps', 'test_component_B'), BUILD_LINUX),
], indirect=True, ids=['linux'])
...
@pytest.mark.host
@pytest.mark.parametrize('embedded_services,target,app_path,build_dir', [
    ('idf', 'linux', os.path.join(HERE, 'tests', 'apps', 'test_integration_AB'), BUILD_LINUX),
], indirect=True, ids=['linux'])
...
@pytest.mark.esp32p4
@pytest.mark.parametrize('embedded_services,target,app_path,build_dir', [
    ('esp,idf', 'esp32p4', os.path.join(HERE, 'tests', 'apps', 'test_integration_AB'), BUILD_ESP32P4),
