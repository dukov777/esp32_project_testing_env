...
7. **Test component directory names must be unique** across the build (`test_a`, `test_b`, `test_int`) — three sibling components all named `test` collide.
...
9. **Each test app's `EXTRA_COMPONENT_DIRS` points at its specific test component dir** (e.g. `test_components/test_a`), not at the `test_components/` parent. On chip targets `set(COMPONENTS …)` isn't used, so listing the parent would auto-include `test_a`/`test_b`/`test_int` in every app and the cross-app mock include wouldn't resolve.
